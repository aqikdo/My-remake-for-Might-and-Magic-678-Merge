...
local function SplitParty()
	...
	if Game.Races[Race].Family == const.RaceFamily.Undead then
		monId = 290
	...
	if not monId then
		...
		if ClassKind == const.ClassKind.Archer then
			monId = Gender == 1 and 477 or 204

		elseif ClassKind == const.ClassKind.Cleric then
			monId = Class == const.Class.PriestDark and 216 or 219

		elseif ClassKind == const.ClassKind.Druid then
			monId = Gender == 1 and 519 or 633

		elseif ClassKind == const.ClassKind.Knight or ClassKind == const.ClassKind.Paladin then
			monId = 261

		elseif ClassKind == const.ClassKind.Monk then
			monId = 585

		elseif ClassKind == const.ClassKind.Ranger then
			monId = 258

		elseif ClassKind == const.ClassKind.Thief then
			monId = Gender == 1 and 15 or 408

		elseif ClassKind == const.ClassKind.Sorcerer then
