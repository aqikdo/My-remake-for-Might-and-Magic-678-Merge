local LogId = "NPCFollowersSkills"
local Log = Log
...
function events.GetSkill(t)
	...
	local NPCSkill, NPCMas = SplitSkill(NPCBonuses.Skills[t.Skill] or CountSkillBonus(t.Skill))
	t.Result = JoinSkill(Skill + NPCSkill, math.max(Mas, NPCMas))
end
...
------------------------------------------------
-- Reputation Rank
local ReputationRankBonuses
local function CalcReputationRankBonus()
	local res = 0
	for k, v in pairs(vars.HiredNPC) do
		local bonus = ReputationRankBonuses[Game.NPC[v].Profession]
		if bonus then
			res = res + bonus
		end
	end
	NPCBonuses.ReputationRank = res
	return res
end

function events.GetReputationRank(t)
	t.Rank = t.Rank + (NPCBonuses.ReputationRank or CalcReputationRankBonus())
end
...
function events.BeforeGotGold(t)
	-- Gold bonuses
	NPCFollowers.LastBonusGold = floor(t.Amount * (NPCBonuses.Gold or CountGoldBonus()))
	...
	if Game.CurrentScreen == 13 then
		...
		local amount
		if MF.GtSettingNum(MM.HirelingCutFromBaseGold, 0) then
			amount = t.Amount
		else
			amount = t.Amount + NPCFollowers.LastBonusGold
		end
		NPCFollowers.LastGoldTaken = min(NPCFollowers.GetTotalFee(amount), amount)
	end
	local new_amount = t.Amount + NPCFollowers.LastBonusGold - NPCFollowers.LastGoldTaken
	Log(Merge.Log.Info, "%s: initial gold: %d, bonus: %d, taken: %d, final gold: %d", LogId,
		t.Amount, NPCFollowers.LastBonusGold, NPCFollowers.LastGoldTaken, new_amount)
	t.Amount = new_amount
end
...
[const.Skills.Merchant]		= {[20] = 4, [21] = 6, [48] = 3, [50] = 8},
[const.Skills.DisarmTraps]	= {[25] = 4, [26] = 6, [51] = 8},
...
ReputationRankBonuses = {[36] = -1, [45] = 1, [48] = 1, [50] = 1, [51] = 1}
