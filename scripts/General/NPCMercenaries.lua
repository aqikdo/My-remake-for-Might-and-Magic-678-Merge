...
local ClassTypesByContinent = {
	[1] = {2, 3, 6, 7, 12, 13, 14}, 		-- Jadame
	[2] = {1, 2, 5, 6, 8, 9, 10, 11, 14}, 	-- Antagarich
...
function GenerateMercenary(t) --RosterId, Class, Level, Skills, Items, Face, JoinText, Condition
	...
	Log(Merge.Log.Info, "GenerateMercenary: RosterId %d, Class %s, Level %d",
			RosterId, Class and tostring(Class) or "nil", Level and tostring(Level) or "nil")
	...
	if Class == nil then
		...
		for k, _ in pairs(table.filter(Game.ClassesExtra, 1, "Kind", ">", 0)) do
			--if k ~= const.Class.Peasant then
				table.insert(allowed_classes, k)
			--end
		end
		Class = allowed_classes[math.random(1, #allowed_classes)]
		Log(Merge.Log.Info, "GenerateMercenary: Class is nil, set to %d", Class)
	end
	Level = Level or 5
	...
	if not Face then
		...
		for i,v in Game.CharacterPortraits do
			if not table.find(PortraitsExceptions[CurCont], i)
					and (v.Race and v.Race >= 0 and Game.Races[v.Race]
					and Game.Races[v.Race].Family ~= const.RaceFamily.Zombie) -- zombie service race
					and (ClassTypes[v.DefClass] == ClassType
					or ClassByRace[v.Race] and ClassByRace[v.Race][ClassStart]) then
	...
	Char.Attrs.Race = Game.CharacterPortraits[Char.Face].Race
	...
	if CurNPC then
		...
		CurQuest.Texts.Ungive = JoinText
				or string.format(Game.NPCText[1680],
						Char.Name, ClassSelfImpressions[math.min(math.ceil(Level/10), 5)],
						GetClassName({ClassId = Char.Class, RaceId = Char.Attrs.Race}))
		if Condition then
...
function events.GameInitialized2()
	...
	for k,v in pairs(Game.ClassesExtra) do
		...
		ClassStages[v.Kind][v.Step + 1] = ClassStages[v.Kind][v.Step + 1] or k
		ClassTypes[k] = v.Kind
	end
...
function events.LoadMap(WasInGame)
	...
	if not WasInGame then
		...
		for k, v in pairs(NPCMercenaries) do
			...
			Quests["Merc" .. k].Texts.Ungive = string.format(Game.NPCText[1680],
					Char.Name, ClassSelfImpressions[math.min(math.ceil(Char.LevelBase/10), 5)],
					GetClassName({ClassId = Char.Class, RaceId = Char.Attrs.Race}))
		end
