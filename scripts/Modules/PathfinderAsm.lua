...
-- takes three points, returns 1 in eax, if they belong to same line.
...
-- Takes three vertexes and destination ptr (16 bytes), returns A, B, C, D params of plane function (Ax + By + Cz + D = 0)
...
local function LoadMapDataBin()
	...
	while ReadNext(5) do
		...
		if CurConverter == 0 then -- straight copy
			...
			pos = pos + CurSize
			Counts[ItemType] = math.floor(CurSize / Props.sizes.V)
		...
		elseif CurConverter == 1 then -- facet
			...
			pos = pos + CurSize
			Counts[ItemType] = math.floor(CurSize / Props.sizes.F)
