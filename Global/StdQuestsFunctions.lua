...
----------------------------------------
-- "Release Archibald"
Game.GlobalEvtLines:RemoveEvent(1343)
evt.Global[1343] = function()
	if not Party.QBits[1201] then	-- NPC
		evt.SetMessage(1731)	-- "Now wait just a minute Tanir, and I’ll make it worth your while to let me…Oh. <Archibald is silent for a long while> I guess I have you people to thank for releasing me from my prison of stone.  Thank you!  You say you need a spell that I've created?  <reaching to the shelves and removing a library scroll> Well, as a reward, I’ll give you the Ritual of the Void.  Use it in good health.  Now, I’m sure you’re very busy, as am I…<Archibald waves his arms and fades away>"
		evt.Add("Inventory", 2164)	-- "Ritual of the Void"
		evt.Subtract("QBits", 1221)	-- Quest item bits for seer
		evt.Add("QBits", 1201)	-- NPC
		evt.Subtract("QBits", 1121)	-- Walt
		evt.Subtract("QBits", 1259)	-- "Obtain Arcane Magic from Archibald in the Royal Library in Castle Ironfist"
		evt.Add("QBits", 1222)	-- Quest item bits for seer
		evt.ForPlayer("All")
		if MM and MF.GtSettingNum(MM.MM6MultibyteReputationBug, 0) then
			evt.Subtract("ReputationIs", 1000 % 256)
		else
			evt.Subtract("ReputationIs", 1000)
		end
		evt.Add("Experience", 50000)
		evt.MoveNPC(797, 0)	-- "Archibald Ironfist"
	end
end
...
--
local TmpBit = 1230
vars.CanClearPilgrimage = vars.CanClearPilgrimage or false
local Pilgrimage = {
...
evt.Global[1354] = function()
	local new_pilgrimage = vars.CanClearPilgrimage

	Party.QBits[TmpBit] = Party.QBits[TmpBit] and not vars.CanClearPilgrimage
	vars.CanClearPilgrimage = false
	if Party.QBits[TmpBit] then
		Message(Game.NPCText[1747])
	else
		local Set = Pilgrimage[Game.Month]
		local hint = strformat(Game.NPCText[1746], Game.GlobalTxt[Set.Month],
			Game.GlobalTxt[Set.Stat], Game.MapStats[Set.Map].Name)
		Message(hint)
		if new_pilgrimage then
			MF.ShowAwardAnimation(93)
		end
	end
end

function events.NewMonth(t)
	-- Uncomment next line to limit pilgrimage to a month
	--Party.QBits[TmpBit] = true
	vars.CanClearPilgrimage = Party.QBits[TmpBit]
end
