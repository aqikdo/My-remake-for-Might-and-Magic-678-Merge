...
local function LloydCheckHook(d, ev, t, IsArena)
	local param1, param2 = d:getparams(0, 2)
	local r = call(mmv(0x4AF370, 0x4CAAF0, 0x4DA920), 0, param1, param2)
	t = t or {}
	t.Map = mem.string(param1):lower()
	if Map:IsIndoor() then
		...
		t.Result = (r ~= 0) -- 将此处 r~=0 改为 r==0 可阻止存挡 
	end
--	t.Result = (r ~= 0)
	if IsArena then
		t[IsArena] = (r == 0)
...
mem.hookfunction(mmv(0x421DC0, 0x427522, 0x425951), 0, 3, function(d, def, mon, kind, dmg)
	...
	if mmver == 8 then
		...
		t.ByPlayer = aptr >= Party.PlayersArray["?ptr"] and  aptr < (Party.PlayersArray["?ptr"] + Party.PlayersArray["?size"])
