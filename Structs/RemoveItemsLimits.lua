...
local LogId = "RemoveItemsLimits"
local Log = Log
Log(Merge.Log.Info, "Init started: %s", LogId)
local MF, MM, MO = Merge.Functions, Merge.ModSettings, Merge.Offsets

local asmpatch, hook, StaticAlloc = mem.asmpatch, mem.hook, mem.StaticAlloc
...
-- Put spell_id into Mod1DiceSides field of ItemsTxt structure
asmpatch(0x454FFE, [[
jnz @end
lea ecx, [ebx+1]
push ecx
...
shl ecx, 4
add ecx, edi
mov byte ptr [ecx+0x23], al
jmp absolute 0x4551FE
@end:
]])
...
local function SetupSpellsHandler()
	...
	MO.SpellScrollBonus = StaticAlloc(4)
	MO.SpellScrollBonusStrength = StaticAlloc(4)
	-- Use item: get spell_id of the scroll
	asmpatch(0x466AED, [[
	mov esi, dword ptr [0xB7CA68]
	mov dword ptr []] .. MO.SpellScrollBonus .. [[], esi
	mov esi, dword ptr [0xB7CA6C]
	mov dword ptr []] .. MO.SpellScrollBonusStrength .. [[], esi
	lea esi, [eax + eax * 2]
	shl esi, 4
	add esi, ]] .. (ItemsTxtPtr + 0x23) .. [[;
	movzx esi, byte ptr [esi]
	test esi, esi
	jz absolute 0x4671CC]])

	-- Setup books

	-- Use item: get spell_id of the book
	asmpatch(0x4669D8, [[
	lea edi, [edx + edx * 2]
	shl edi, 4
	add edi, ]] .. (ItemsTxtPtr + 0x23) .. [[;
...
local function RemoveItemsLimits()
	...
	add esp, 0xc
	push ]] .. NewSpcCount .. [[;]])
	if NewSpcCount < 128 then
		mem.u1[0x45516D + 3] = NewSpcCount
	else
		mem.asmpatch(0x45516D, [[
		cmp dword ptr [ebp-0xC], ]] .. NewSpcCount .. [[;
		jl absolute 0x455155]])
	end
	...
	if NewStdCount < 128 then
		mem.u1[0x455128 + 3] = NewStdCount
	else
		mem.asmpatch(0x455128, [[
		cmp dword ptr [ebp-0xC], ]] .. NewStdCount .. [[;
		jl absolute 0x455110]])
	end
	...
	TmpAddrTable = {	-- 0x5efbe0 ptrs
					...
					0x444422, 0x4665e3, 0x467479, 0x467aea, 0x48cf01, 0x49002f, 0x49104f, 0x494d98, 0x4bb629,
						-- 0x5efbe9 ptrs
					mem.u4[0x42db92] + 5 + 0x42db91, -- MMPatch 2.5 specific 
					0x43718c, 0x437b8b, 0x43b7cc, 0x467483, 0x48cf26, 0x48d73a, 0x48d797, 0x48d848, 0x48f150, 0x48f24a,
...
local function ExtraItemsInShops()
	...
	-- Use events.GetShopItemTreatment to overwrite
	asmpatch(0x49003A, [[
	push edx
	imul edx, 0x30
	add edx, ]] .. (NewSpacePtr + 4) .. [[;

	cmp dword [edx+0x10], 0
	jle @neq

	cmp byte [edx+0x21], 3
	jl]] .. (MF.GtSettingNum(MM.PermitSellSpecials, 0) and "e" or "") .. [[ @end

	@neq:
	pop edx
	push 5
	jmp ]] .. (0x490096 - 0x49003A) .. [[;
	@end:
	pop edx
	]], 0x490068 - 0x49003A)

	-- events.CanShopOperateOnItem hook was broken with extra houses.
	-- Should be fixed in MMExtension 137d746a (2022-04-09)
	asmpatch(0x4BB630, [[
	push esi
	imul esi, 0x30
	add esi, ]] .. (NewSpacePtr + 4) .. [[;
